...
function BrandMark({
  ...
  const vendor = resolveVendor(model.vendor, model.name);
  ...
  return (
    <VendorMark
      vendor={vendor}
      title={vendorDisplayName(vendor)}
...
function ModelCard({
  ...
  const vendor = resolveVendor(model.vendor, model.name);
  const tone = vendorTone(vendor ?? model.vendor);
  const label = vendor ? vendorDisplayName(vendor) : null;
    ...
    <SceneItem delay={delay} from={featured ? "scale" : rank % 2 === 0 ? "left" : "right"}>
      <TiltCard
        ...
        className={cx(
          ...
          featured ? "border-accent/40 p-5 sm:p-6 lg:p-8" : "p-4 sm:p-5",
        )}
      >
        {featured ? (
          <BrandMark
